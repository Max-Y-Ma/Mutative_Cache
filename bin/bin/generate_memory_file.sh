...
# Setup appropriate linker scripts for spike
if [ "$2" -eq 1 ]; then
  echo "Setting linker script and startup.s for SPIKE"
  LINK_FILE=$SH_LOCATION/spike_link.ld
  START_FILE=$SH_LOCATION/spike_startup.s
else
  echo "Setting linker script and startup.s for TEST"
...
function ecompile {
    ...
    MAP_FILE="${WORK_DIR}/$(basename "${IN_FILE%.*}").map"
    rm -f $ELF_FILE
    rm -f $MAP_FILE
    if [ -z "$INC_FILE" ]; then
        "$ASSEMBLER" -mcmodel=medany -static -fno-common -ffreestanding -nostartfiles -march=$ARCH -mabi=ilp32 -Ofast -flto -Wall -Wextra -Wno-unused -Wl,-Map,"$MAP_FILE" -T$LINK_FILE $START_FILE "${WORK_DIR}/$(basename $IN_FILE)" -o "$ELF_FILE" -lm -static-libgcc -lgcc -lc -Wl,--no-relax
    else
        "$ASSEMBLER" -mcmodel=medany -static -fno-common -ffreestanding -nostartfiles -march=$ARCH -mabi=ilp32 -Ofast -flto -Wall -Wextra -Wno-unused -Wl,-Map,"$MAP_FILE" -T$LINK_FILE $START_FILE "${WORK_DIR}/$(basename $IN_FILE)" -o "$ELF_FILE" -lm -static-libgcc -lgcc -lc -Wl,--no-relax -I"$(basename $INC_FILE)"
    fi
